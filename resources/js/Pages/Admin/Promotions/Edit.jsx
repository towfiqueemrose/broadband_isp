...
export default function Edit({ promotion }) {
    const form = useForm({
        eyebrow: promotion.eyebrow || '', title: promotion.title, description: promotion.description || '',
        cta_label: promotion.cta_label || '', cta_url: promotion.cta_url || '', image: null,
        display_location: promotion.display_location, start_date: promotion.start_date || '', end_date: promotion.end_date || '',
        is_active: promotion.is_active, sort_order: promotion.sort_order,
        _detailsText: Array.isArray(promotion.details) ? promotion.details.join('\n') : '',
    });
    const submit = (e) => {
        e.preventDefault();
        const data = { ...form.data };
        if (data._detailsText) data.details = data._detailsText.split('\n').filter(d => d.trim());
        delete data._detailsText;
        form.put(route('admin.promotions.update', promotion.id), { data, forceFormData: true });
    };
    return (
        <AdminLayout title={`Edit — ${promotion.title}`}>
            <PageHeader title={`Edit ${promotion.title}`} actionHref={route('admin.promotions.index')} actionLabel="Back" />
            <form onSubmit={submit} encType="multipart/form-data" className="max-w-3xl space-y-6">
                <FormCard title="Promotion Content">
                    <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        <div className="sm:col-span-2"><FormTextarea label="Details (one per line)" value={form.data._detailsText} onChange={(e) => form.setData('_detailsText', e.target.value)} rows={4} /></div>
                    </div>
                </FormCard>
                ...
                <button type="submit" disabled={form.processing} className="inline-flex items-center gap-2 rounded-xl bg-primary px-6 py-2.5 text-sm font-semibold text-white hover:bg-primary-dark disabled:opacity-50">
                    {form.processing ? 'Saving...' : 'Update Promotion'}
                </button>
            </form>
        </AdminLayout>
    );
