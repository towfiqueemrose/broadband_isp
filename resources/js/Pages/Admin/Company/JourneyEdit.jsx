...
export default function Edit({ milestone }) {
    const form = useForm({ year: milestone.year, title: milestone.title, description: milestone.description || '', is_active: milestone.is_active, sort_order: milestone.sort_order });
    const submit = (e) => { e.preventDefault(); form.put(route('admin.company.journey.update', milestone.id)); };
    return (
        <AdminLayout title="Edit Milestone">
            <PageHeader title="Edit Milestone" actionHref={route('admin.company.journey.index')} actionLabel="Back" />
            <form onSubmit={submit} className="max-w-3xl space-y-6">
                <FormCard>
                    <div className="grid gap-4 sm:grid-cols-2">
                        <FormInput label="Year" value={form.data.year} onChange={(e) => form.setData('year', e.target.value)} required />
