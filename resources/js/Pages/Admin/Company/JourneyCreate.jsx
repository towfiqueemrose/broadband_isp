...
export default function Create() {
    const form = useForm({ year: '', title: '', description: '', is_active: true, sort_order: 0 });
    const submit = (e) => { e.preventDefault(); form.post(route('admin.company.journey.store')); };
    return (
        <AdminLayout title="Add Milestone">
            <PageHeader title="Add Milestone" actionHref={route('admin.company.journey.index')} actionLabel="Back" />
            <form onSubmit={submit} className="max-w-3xl space-y-6">
                <FormCard>
                    <div className="grid gap-4 sm:grid-cols-2">
                        <FormInput label="Year" value={form.data.year} onChange={(e) => form.setData('year', e.target.value)} error={form.errors.year} required placeholder="2024" />
