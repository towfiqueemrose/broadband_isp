...
export default function Edit({ faq }) {
    const form = useForm({ question: faq.question, answer: faq.answer, category: faq.category || '', display_location: faq.display_location || 'all', is_active: faq.is_active, sort_order: faq.sort_order });
    const submit = (e) => { e.preventDefault(); form.put(route('admin.faqs.update', faq.id)); };

    return (
        <AdminLayout title="Edit FAQ">
            <PageHeader title="Edit FAQ" actionHref={route('admin.faqs.index')} actionLabel="Back" />
            <form onSubmit={submit} className="max-w-3xl space-y-6">
                <FormCard>
                    <div className="space-y-4">
                        ...
                        <div className="grid gap-4 sm:grid-cols-3">
                            <FormInput label="Category" value={form.data.category} onChange={(e) => form.setData('category', e.target.value)} />
                            <FormSelect label="Display Location" value={form.data.display_location} onChange={(e) => form.setData('display_location', e.target.value)} options={[{ value: 'all', label: 'All Pages' }, { value: 'homepage', label: 'Homepage' }, { value: 'contact', label: 'Contact Page' }, { value: 'packages', label: 'Packages Page' }]} />
                ...
                <button type="submit" disabled={form.processing} className="inline-flex items-center gap-2 rounded-xl bg-primary px-6 py-2.5 text-sm font-semibold text-white hover:bg-primary-dark disabled:opacity-50">
                    {form.processing ? 'Saving...' : 'Update FAQ'}
                </button>
            </form>
        </AdminLayout>
    );
