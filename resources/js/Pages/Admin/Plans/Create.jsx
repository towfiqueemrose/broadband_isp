...
export default function Create({ categories }) {
    ...
    return (
        <AdminLayout title="Create Plan">
            <PageHeader title="Create Plan" description="Add a new internet package." actionHref={route('admin.plans.index')} actionLabel="Back" />
            <form onSubmit={submit} className="max-w-4xl space-y-6">
                ...
                <FormCard title="Pricing & Speed">
                    <div className="grid gap-4 sm:grid-cols-3">
                        <FormInput label="Download (Mbps)" type="number" value={form.data.download_mbps} onChange={(e) => form.setData('download_mbps', e.target.value)} error={form.errors.download_mbps} required />
                        <FormInput label="Upload (Mbps)" type="number" value={form.data.upload_mbps} onChange={(e) => form.setData('upload_mbps', e.target.value)} error={form.errors.upload_mbps} description="Defaults to download speed" />
                        <FormInput label="Monthly Price (৳)" type="number" value={form.data.price_monthly} onChange={(e) => form.setData('price_monthly', e.target.value)} error={form.errors.price_monthly} required />
                        ...
                        <FormInput label="Billing Label" value={form.data.billing_label} onChange={(e) => form.setData('billing_label', e.target.value)} placeholder="per month" />
                    </div>
                </FormCard>
                ...
                <FormCard title="Promotion">
                    <div className="grid gap-4 sm:grid-cols-3">
                        ...
                        <FormInput label="Promo Label" value={form.data.promo_label} onChange={(e) => form.setData('promo_label', e.target.value)} placeholder="Limited Time" />
